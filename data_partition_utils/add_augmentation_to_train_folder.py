...
def add_aug_to_folder(data_name, data_dir, is_kfold, aug_name, augmentation_dir, augmentation_data_dir_output):
    ...
    if is_kfold == True:
        d_without_kfold, data_quality, n_k = data_name.rsplit('_', 2)
        ...
        imgs_dir_output_path = path.join(augmentation_data_dir_output, 'imgs', f'{data_name}_{aug_name_tag}')
        masks_dir_output_path = path.join(augmentation_data_dir_output, 'masks', f'{data_name}_{aug_name_tag}')
